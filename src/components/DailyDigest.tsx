...
export default function DailyDigest() {
  ...
  useEffect(() => {
    invoke<DigestData>("get_daily_digest")
      .then(setData)
      .catch(() => setData({ overdue: 0, due_today: 0, unfinished_yesterday: 0, total_active: 0 }));
  }, []);
  ...
  function getMessage() {
    ...
    if (data.due_today > 0) parts.push(`${data.due_today} due today`);
    if (parts.length === 0) return "You're all caught up. Great start to the day.";
    return parts.join(", ") + ".";
  }
      ...
      {visible && (
        ...
        >
                ...
                <div>
                  <div
                    style={{
                      fontSize: "20px",
                      fontWeight: 700,
                      color: data.due_today > 0 ? "var(--text-primary)" : "var(--text-muted)",
                    }}
                  >
                    {data.due_today}
                  </div>
                ...
                <div>
                  <div
                    style={{
                      fontSize: "20px",
                      fontWeight: 700,
                      color: data.overdue > 0 ? "var(--text-primary)" : "var(--text-muted)",
                    }}
                  >
                    {data.overdue}
                  </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={handleDismiss} className="feature-btn" style={{ padding: "5px 12px" }}>
              Dismiss {countdown}s
            </button>
          </div>
        </motion.div>
